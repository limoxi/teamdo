...
const props = defineProps(['task', 'userId2avatar'])
...
const userAvatars = computed(() => props.task.users.map(user => {
  return {
    src: props.userId2avatar[user.id] || defaultAvatar,
    tip: user.nickname
  }
}))
...
.aui-task {
  ...
  overflow: hidden;
  width: 280px;
  box-shadow: 1px 2px 8px 0 #808695;
